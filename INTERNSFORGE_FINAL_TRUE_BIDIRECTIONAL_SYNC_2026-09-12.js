...
function syncCounselorRowToFirebase(counselorSheet, rowNumber) {
  ...
  // Standardize Call Status before syncing counselor edits.
  app["Call Status"] = standardizeCallStatus_(app["Call Status"]);
  ensureCallStatusDropdown_(counselorSheet);
...
function syncSheetRowToFirebase(rowNumber, previousAssignedTo) {
  ...
  /*
   * If a legacy status was typed/selected, immediately correct
   * the Master Sheet cell to the canonical value.
   */
  ...
  if (
    statusIndex >= 0 &&
    value(row[statusIndex]) !== app["Call Status"]
  ) {
    sheet
      .getRange(rowNumber, statusIndex + 1)
      .setValue(app["Call Status"]);
  }
...
function testSelectedMasterLeadSync() {
  ...
  if (!activeRange) {
    throw new Error(
      "Select an application row in Master Sheet1 first."
    );
  }
  ...
  if (rowNumber < 2) {
    throw new Error(
      "Select a lead row, not the header row."
    );
  }

  const result =
    syncSheetRowToFirebase(
      rowNumber,
      ""
    );
...
function getHeaders(sheet) {
  ...
  // The master Counselors registry may use Google Sheets typed columns.
  // Use the configured header names instead of reading those typed cells.
  if (
    typeof COUNSELOR_CONFIG !== "undefined" &&
    sheet.getName() === COUNSELOR_CONFIG.LIST_SHEET_NAME
  ) {
  ...
  try {
    ...
    console.log(
      "getHeaders skipped for " + sheet.getName() + ": " + error.message
    );
    return [];
  }
}
