...
let crmFilterSignature = "";

function populateCrmFilters(force = false) {
  ...
  const signature = `${domains.join("\u001f")}\u001e${years.join("\u001f")}\u001e${counselorNames.join("\u001f")}`;
  if (!force && signature === crmFilterSignature) return;
  crmFilterSignature = signature;
...
function promptForCounselor() {
  ...
  renderApplicationCRM({ refreshCounselors: true });
  return clean;
}
...
async function assignApplicationToCounselor(appId, counselor, source = "row") {
  ...
  try {
    ...
    renderApplicationCRM({ refreshFilters: false });
...
let crmCounselorControlsReady = false;

function populateCrmCounselorControls(force = false) {
  if (crmCounselorControlsReady && !force) return;
  loadCounselorNames();
  ...
  crmCounselorControlsReady = true;
}

function renderApplicationCRM(options = {}) {
  loadCounselorNames();
  populateCrmCounselorControls(Boolean(options.refreshCounselors));
  if (options.refreshFilters !== false) populateCrmFilters();
...
function renderCrmTable(rows) {
  ...
  const counselorOptionCache = new Map();
  const getCounselorOptions = assigned => {
    const key = String(assigned || "");
    if (!counselorOptionCache.has(key)) counselorOptionCache.set(key, counselorOptions(key));
    return counselorOptionCache.get(key);
  };
  body.innerHTML = rows.slice(0, 100).map(app => {
    ...
    return `<tr>
      ...
      <td><select class="crm-assign-select" data-app-id="${esc(app.id)}" aria-label="Assign ${esc(app.name || "lead")}">${getCounselorOptions(assigned)}</select></td>
  ...
  // Delegate CRM row interactions once instead of attaching handlers to every row on every render.
  if (!body.dataset.delegated) {
    ...
    body.addEventListener("click", event => {
      ...
      if (button.classList.contains("crm-open-btn") || button.classList.contains("crm-history-btn")) {
        openApplicationModal(id);
      } else if (button.classList.contains("crm-delete-btn")) {
        deleteSingleApplication(id);
      }
    });
    body.addEventListener("change", event => {
      const target = event.target;
      if (target.classList.contains("crm-row-check")) {
        toggleCrmSelection(target.dataset.appId, target.checked);
        return;
      }
      if (target.classList.contains("crm-assign-select")) {
        handleCounselorSelect(target, value => assignApplicationToCounselor(target.dataset.appId, value, "row"));
      }
    });
  }
  updateCrmSelectionUI();
...
function setupUI() {
  ...
  const scheduleCrmFilter = event => {
    const id = event.currentTarget?.id || "";
    // Selects can update immediately; text search is debounced so typing never rebuilds
    // the 100-row CRM table for every keystroke.
    if (id === "crmSearch") {
      window.clearTimeout(crmFilterTimer);
      crmFilterTimer = window.setTimeout(filterCrmApplications, 120);
    } else {
      filterCrmApplications();
    }
  };
  ["crmSearch","crmStatusFilter","crmDomainFilter","crmYearFilter","crmCounselorFilter","crmFollowupFilter"].forEach(id => {
    const node = el(id);
    node?.addEventListener("input", scheduleCrmFilter);
    node?.addEventListener("change", scheduleCrmFilter);
  });
